...
export default function StatusManager({
  ...
  // Inline confirmation dialog (remplace window.confirm() bloquant)
  ...
  const handleAction = (action: StatusAction) => {
    if (action.confirmMessage) {
      // Ouvre le dialog React inline — pas de window.confirm() bloquant
  ...
  // ── Dialog de confirmation inline (remplace window.confirm) ──────────────
  const ConfirmDialog = (
    <Modal isOpen={!!pendingAction} onClose={() => setPendingAction(null)} size="sm">
      <div className="text-center">
        <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-amber-100">
          <AlertTriangle className="h-6 w-6 text-amber-600" aria-hidden="true" />
        </div>
        <h3 className="text-base font-bold text-gray-900 mb-4">
          {pendingAction?.confirmMessage ?? 'Confirmer cette action ?'}
        </h3>
        <div className="flex gap-3">
          ...
          <button
            ...
            className="flex-1 px-4 py-2.5 rounded-xl text-sm font-bold text-white bg-brand-600 hover:bg-brand-700 transition-colors"
          >
            Confirmer
          </button>
        </div>
  ...
  if (actions.length === 0 && onDelete) {
    // Aucune transition de statut mais un bouton supprimer
    return (
      <>
        {ConfirmDialog}
