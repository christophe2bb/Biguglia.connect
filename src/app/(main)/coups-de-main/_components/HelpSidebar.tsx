...
import { MapPin, Zap, Filter, Bookmark, Shield, HandHeart, X, ChevronRight, Flame, Info, ArrowRight } from 'lucide-react';
          ...
          {SECTORS.map(sector => {
            ...
            return (
              <button key={sector.id} type="button"
                ...
                <span className={`text-xs font-bold px-1.5 py-0.5 rounded-full ${isActive ? colors.badge : 'bg-gray-100 text-gray-500'}`}>
                  {count}
                </span>
              </button>
            );
