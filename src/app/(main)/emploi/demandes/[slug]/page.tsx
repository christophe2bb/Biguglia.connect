...
export async function generateMetadata({ params }: PageProps) {
  ...
  const title = `${demand.title} – ${demand.location_label} | Biguglia Connect`;
  const description = demand.short_description ?? '';
  return {
    title,
    description,
    alternates: { canonical: `${SITE_URL}/emploi/demandes/${slug}` },
    openGraph: {
      title,
      description,
      url: `${SITE_URL}/emploi/demandes/${slug}`,
    },
  };
}
                  ...
                  <div className="flex items-center gap-3">
                    {demand.author_profile.avatar_url ? (
                      <div className="relative w-12 h-12 rounded-full overflow-hidden flex-shrink-0">
                        <Image src={demand.author_profile.avatar_url} alt={demand.author_profile.display_name} fill sizes="48px" className="rounded-full object-cover" />
                      </div>
                    ) : (
