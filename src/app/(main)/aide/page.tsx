  ...
  return (
    <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      ...
      <section aria-labelledby="faq-heading" className="mb-12">
        {/* Titre visible uniquement par les lecteurs d'écran */}
          ...
          {FAQ_ITEMS.map((item, index) => (
            ...
            >
              <summary className="flex items-center justify-between p-5 cursor-pointer font-medium text-gray-900 hover:text-brand-600 transition-colors">
                ...
                <span
                  aria-hidden="true"
                  className="text-gray-400 group-open:rotate-180 transition-transform duration-200 ml-4 flex-shrink-0"
                >
                  ▼
                </span>
              </summary>
      ...
      {/* ── Section contact ── */}
      <section
        aria-labelledby="contacter-heading"
        id="contacter"
        className="bg-brand-50 rounded-2xl p-6 text-center"
      >
        {/* Titre visible uniquement par les lecteurs d'écran */}
        <h2 id="contacter-heading" className="sr-only">Nous contacter</h2>
        <p className="text-brand-800 font-medium mb-2">
          Vous n&apos;avez pas trouvé votre réponse&nbsp;?
        </p>
        <p className="text-brand-600 text-sm mb-4">
          Contactez l&apos;administrateur par email ou via le forum.
        </p>
        <Link
          href="/forum"
          className="inline-flex items-center justify-center bg-brand-600 text-white px-5 py-2.5 rounded-xl text-sm font-medium hover:bg-brand-700 transition-colors"
        >
