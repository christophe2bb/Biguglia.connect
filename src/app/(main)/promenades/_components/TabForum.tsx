...
export default function TabForum({
  ...
  // Si un post est sélectionné → afficher la vue détail
  if (selectedPost) {
    return (
      <ForumPostDetail
        post={selectedPost}
  ...
  return (
    <div>
      ...
      {showPostForm && profileId && (
        <form onSubmit={handlePostSubmit} className="bg-white rounded-2xl border border-sky-200 p-5 mb-6 shadow-sm">
          ...
          <div className="flex gap-2 mt-3">
            ...
            <button type="button" onClick={() => setShowPostForm(false)} className="px-5 py-2 rounded-xl text-sm text-gray-500 hover:bg-gray-100 border border-gray-200">
              Annuler
            </button>
          </div>
        </form>
      ...
      {/* ── États de chargement / vide / liste ── */}
      {loadingForum ? (
        ...
        <div className="bg-sky-50 border border-sky-200 rounded-2xl p-8 text-center">
          ...
          <p className="text-sky-700 text-sm">
            Exécutez <code className="bg-sky-100 px-1 rounded font-mono text-xs">migration_themes.sql</code> dans Supabase pour activer ce forum.
          </p>
        </div>
      ) : forumPosts.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
          ...
          <p className="text-gray-400 text-sm mb-5">Posez une question ou partagez votre expérience !</p>
          {profileId && (
            <button
        ...
        <div className="space-y-3">
          {forumPosts.map(post => {
            ...
            return (
              /* ── Bouton au lieu de Link → ouvre inline ── */
              <button
                key={post.id}
                type="button"
              ...
              >
                <div className={cn(
                  ...
                  <MessageSquare className={cn('w-5 h-5', isHot ? 'text-amber-500' : 'text-sky-400')} />
                </div>
                ...
                {/* Flèche */}
                <svg className="w-4 h-4 text-gray-300 group-hover:text-sky-400 flex-shrink-0 mt-1 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {!profileId && (
        <div className="mt-6 bg-sky-50 border border-sky-200 rounded-2xl p-6 text-center">
          ...
          <a href="/connexion" className="inline-flex items-center gap-2 bg-sky-500 text-white font-bold px-5 py-2.5 rounded-xl text-sm hover:bg-sky-600 transition-colors">
            Se connecter
          </a>
        </div>
      )}
