...
import { ChevronLeft, MapPin, Calendar, MessageSquare, Flag, Shield, Clock } from 'lucide-react';
...
export default function MaterielDetailPage() {
  ...
  useEffect(() => {
    const fetchItem = async () => {
      ...
      const { data, error } = await supabase
        ...
        .select(`
          *,
          owner:profiles!equipment_items_owner_id_fkey(id, full_name, avatar_url),
          category:equipment_categories(*),
          photos:equipment_photos(id, url, display_order)
        `)
        .eq('id', id as string)
        .single();
      ...
      setItem(data as unknown as EquipmentItem);
      setLoading(false);
    };
    fetchItem();
  }, [id, router]);
  ...
  const handleBorrowRequest = async () => {
    if (!profile) {
      router.push('/connexion?redirect=/materiel/' + id);
      return;
    }
    if (!borrowForm.start_date || !borrowForm.end_date) {
    ...
    const { error } = await supabase.from('borrow_requests').insert({
      ...
      start_date: borrowForm.start_date,
      end_date: borrowForm.end_date,
      message: borrowForm.message,
      status: 'pending',
    });

    if (error) {
      toast.error('Erreur lors de la demande. Réessayez.');
    } else {
      toast.success('Demande d\'emprunt envoyée ! Le propriétaire sera notifié.');
      setBorrowForm({ start_date: '', end_date: '', message: '' });
    }
    setSubmitting(false);
  };
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5">
            ...
            <div className="flex items-center gap-3 mb-4">
              <Avatar src={item.owner?.avatar_url} name={item.owner?.full_name || '?'} size="md" />
              <div>
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Précisez votre usage..."
                    className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm resize-none h-20 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-transparent"
                  />
                </div>
          ...
          {isOwner && (
            <div className="bg-gray-50 rounded-2xl border border-gray-200 p-4 text-center text-sm text-gray-500">
