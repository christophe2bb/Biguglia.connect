...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const { data: pending } = await supabase
        ...
        .select(`
          id, user_id, business_name, created_at,
          profile:profiles!artisan_profiles_user_id_fkey(id, full_name, email, avatar_url, status, role, created_at),
          trade_category:trade_categories(name, icon)
        `)
        .eq('profiles.role', 'artisan_pending')
        .order('created_at', { ascending: true });

      setPendingArtisans((pending as unknown as PendingArtisan[]) || []);
      setLoading(false);
    };
  ...
  const approveArtisan = async (artisanUserId: string, artisanProfileId: string) => {
    const supabase = createClient();
  ...
  const rejectArtisan = async (artisanUserId: string) => {
    if (!confirm('Refuser et supprimer ce profil artisan ?')) return;
    const supabase = createClient();
    ...
    await supabase.from('notifications').insert({
      ...
      message: 'Votre demande d\'inscription en tant qu\'artisan n\'a pas été acceptée. Contactez-nous pour plus d\'informations.',
      link: '/aide',
    });
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm">Tableau de bord webmaster — Biguglia Connect</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
        {[
          ...
          <Link key={href} href={href}>
            <div className="bg-white rounded-2xl border border-gray-100 p-5 hover:shadow-sm hover:border-gray-200 transition-all duration-200">
      ...
      <div>
        <h2 className="text-lg font-semibold text-gray-900 mb-4">
          Artisans en attente de validation
          {stats?.pending_artisans ? (
            <Badge variant="warning" className="ml-2">{stats.pending_artisans} en attente</Badge>
          ) : null}
        </h2>
              ...
              <div key={artisan.id} className="bg-white rounded-2xl border border-orange-200 p-5">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-gray-400 mt-0.5">
                        {artisan.trade_category?.icon} {artisan.trade_category?.name} · Demandé {formatRelative(artisan.created_at)}
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    <Button
                      size="sm"
                      onClick={() => approveArtisan(artisan.user_id, artisan.id)}
                    >
                      ✅ Valider
                    </Button>
                  </div>
