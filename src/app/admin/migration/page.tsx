...
export default function MigrationPage() {
  ...
  const [copiedForumV2,   setCopiedForumV2]   = useState(false);
  const [copiedSectors,   setCopiedSectors]   = useState(false);
...
SELECT 'OK: statuts enrichis appliqués avec succès' AS result;`;
        ...
        <div className="flex items-center gap-3 px-5 py-4 bg-gradient-to-r from-teal-600 to-emerald-600">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-teal-100 mt-0.5">
              ...
              (perdu/trouvé, coups de main, événements, promenades, associations, collectionneurs, matériel, annonces)
              et une vue <code className="bg-white/20 px-1 rounded">sector_stats</code> pour les statistiques.
            </p>
          </div>
          <button
            onClick={() => {
              navigator.clipboard.writeText(SECTORS_SQL).then(() => {
                setCopiedSectors(true);
                setTimeout(() => setCopiedSectors(false), 3000);
              });
            }}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold transition-colors ml-4 flex-shrink-0 ${
      ...
      <div className="mt-6 bg-gray-900 rounded-2xl overflow-hidden border border-indigo-700">
        <div className="flex items-center justify-between px-4 py-3 bg-indigo-900/60 border-b border-indigo-700">
          ...
          <button
            onClick={() => {
              navigator.clipboard.writeText(FORUM_V2_SQL).then(() => {
                setCopiedForumV2(true);
                setTimeout(() => setCopiedForumV2(false), 3000);
              });
            }}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold transition-colors ml-4 flex-shrink-0 ${
