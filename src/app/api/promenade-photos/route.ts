 ...
 *
 * DELETE /api/promenade-photos
 * ──────────────────────────────────────────────────────────────────────────────
 * Supprime une photo d'un itinéraire. Utilise le client admin pour bypasser
 * les RLS. Vérifie que l'utilisateur est bien l'auteur de la promenade.
 *
 * Sécurité :
...
export async function DELETE(req: NextRequest): Promise<NextResponse> {
  ...
  // ③ Parse body
  let body: { promenade_id: string; url: string };
  try {
    body = await req.json();
  } catch {
    return NextResponse.json({ error: 'Corps JSON invalide' }, { status: 400 });
