  ...
  return (
    <svg viewBox="0 0 520 420" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
      ...
      {[0,45,90,135,180,225,270,315].map((angle, i) => (
        <line key={i}
          x1={420 + Math.cos(angle * Math.PI / 180) * 50} y1={70 + Math.sin(angle * Math.PI / 180) * 50}
          x2={420 + Math.cos(angle * Math.PI / 180) * 62} y2={70 + Math.sin(angle * Math.PI / 180) * 62}
          stroke="#fbbf24" strokeWidth="2.5" strokeLinecap="round" opacity="0.6"
        />
      ))}
    </svg>
  );
  ...
  return (
    <svg viewBox="0 0 400 300" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
      ...
      {[0,1,2,3].map(row => [0,1,2,3,4,5].map(col => (
        <rect key={`${row}-${col}`} x={68 + col * 44} y={68 + row * 37} width="40" height="33"
          fill={row % 2 === col % 2 ? "#f1f5f9" : "#e8f0fe"} rx="2" stroke="#dde3ec" strokeWidth="0.5" />
      )))}
      ...
      <circle cx="190" cy="91" r="4" fill="#1e293b" /><circle cx="210" cy="91" r="4" fill="#1e293b" />
      <circle cx="191" cy="89" r="1.5" fill="white" /><circle cx="211" cy="89" r="1.5" fill="white" />
      ...
      <g transform="translate(260, 108)">
        ...
        <circle cx="7" cy="30" r="6" fill="#1f2937" /><circle cx="28" cy="30" r="6" fill="#1f2937" />
...
function CommunityIllustration() {
  return (
    <svg viewBox="0 0 400 280" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
      ...
      {[{ cx: 110, cy: 210, color: "#fdba74" }, { cx: 200, cy: 225, color: "#a78bfa" }, { cx: 290, cy: 215, color: "#6ee7b7" }].map(({ cx, cy, color }, i) => (
        <g key={i}>
  ...
  return (
    <svg viewBox="0 0 400 280" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
      ...
      <g transform="translate(60, 20)">
        ...
        <circle cx="8" cy="36" r="7" fill="#1f2937" /><circle cx="37" cy="36" r="7" fill="#1f2937" />
      ...
      <g transform="translate(215, 22)">
        ...
        <circle cx="40" cy="15" r="5" fill="#e0f2fe" /><circle cx="40" cy="15" r="2" fill="#0ea5e9" />
      ...
      <g transform="translate(265, 88)">
        ...
        <circle cx="25" cy="52" r="10" fill="#374151" /><circle cx="55" cy="52" r="10" fill="#374151" />
        <text x="8" y="72" fontSize="9" fill="#374151">Nettoyeur HP</text>
      </g>
      <g transform="translate(170, 195)">
        <path d="M0 15 Q30 0 60 15" stroke="#10b981" strokeWidth="2.5" strokeLinecap="round" fill="none" />
...
const sections = [
  {
    icon: Wrench, title: 'Marketplace artisans', subtitle: 'Artisans vérifiés',
    description: 'Trouvez un plombier, électricien, maçon ou tout autre artisan local qualifié et validé par notre équipe.',
    href: '/artisans', gradient: 'from-orange-500 to-red-600', accentColor: 'text-orange-600', accentBg: 'bg-orange-50',
    Illustration: ArtisanIllustration,
  },
  {
    icon: Package, title: 'Petites annonces', subtitle: 'Locaux & gratuit',
    description: 'Vendez, cherchez ou offrez du matériel et des services à vos voisins. Simple et efficace.',
    href: '/annonces', gradient: 'from-blue-500 to-indigo-600', accentColor: 'text-blue-600', accentBg: 'bg-blue-50',
    Illustration: CommunityIllustration,
  },
  {
    icon: Package, title: 'Prêt de matériel', subtitle: 'Entre voisins',
    description: 'Empruntez une perceuse, un escabeau, une remorque… sans acheter. La solidarité de quartier.',
    href: '/materiel', gradient: 'from-green-500 to-teal-600', accentColor: 'text-green-600', accentBg: 'bg-green-50',
    Illustration: MaterialIllustration,
  },
  {
    icon: BookOpen, title: 'Forum communauté', subtitle: 'Entraide locale',
    description: 'Discutez, recommandez et échangez avec les habitants de Biguglia. La vie de quartier en ligne.',
    href: '/forum', gradient: 'from-purple-500 to-violet-600', accentColor: 'text-purple-600', accentBg: 'bg-purple-50',
    Illustration: ForumIllustration,
  },
];

const steps = [
  { num: '01', emoji: '📝', title: 'Décrivez votre besoin', desc: 'Sélectionnez le type de travaux, rédigez votre demande et ajoutez des photos pour être plus précis.', color: 'from-orange-400 to-orange-600', bg: 'bg-orange-50', border: 'border-orange-200' },
  { num: '02', emoji: '💬', title: 'Contactez un artisan', desc: 'Parcourez les profils vérifiés, envoyez votre demande et échangez par messagerie sécurisée.', color: 'from-blue-400 to-blue-600', bg: 'bg-blue-50', border: 'border-blue-200' },
...
function AnimatedCount({ target, suffix = '', loading }: { target: number; suffix?: string; loading: boolean }) {
  ...
  if (loading) return <Loader2 className="w-6 h-6 animate-spin text-gray-300 inline" />;
...
function StatCard({ value, label, icon: Icon, color, bg, border, suffix, loading }: StatItem) {
  return (
    <div className="flex flex-col items-center text-center">
      <div className={`w-14 h-14 rounded-2xl ${bg} ${border} border-2 flex items-center justify-center mb-3 shadow-sm`}>
        <Icon className={`w-7 h-7 ${color}`} />
      </div>
      <div className={`text-3xl sm:text-4xl font-black ${color} mb-1 tabular-nums`}>
        <AnimatedCount target={value} suffix={suffix} loading={loading} />
      </div>
...
export default function HomePage() {
  ...
  const stats: StatItem[] = [
    { value: counts.artisans, suffix: '', label: 'Artisans vérifiés', icon: Shield, color: 'text-brand-600', bg: 'bg-brand-50', border: 'border-brand-100', loading: countsLoading },
    { value: counts.membres, suffix: '', label: 'Membres inscrits', icon: Users, color: 'text-blue-600', bg: 'bg-blue-50', border: 'border-blue-100', loading: countsLoading },
    { value: counts.annonces, suffix: '', label: 'Annonces actives', icon: Package, color: 'text-amber-600', bg: 'bg-amber-50', border: 'border-amber-100', loading: countsLoading },
    { value: 0, suffix: '€', label: 'Inscription gratuite', icon: CheckCircle, color: 'text-green-600', bg: 'bg-green-50', border: 'border-green-100', loading: false },
  ];
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 w-full">
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-8 items-center">
            <div>
              ...
              <h1 className={`text-5xl sm:text-6xl lg:text-7xl font-black text-gray-900 leading-[1.02] mb-6 transition-all duration-700 delay-100 ${heroLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
                ...
                <span className="relative">
                  ...
                  <svg className="absolute -bottom-3 left-0 w-full h-3" viewBox="0 0 300 10" preserveAspectRatio="none">
                    ...
                    <defs><linearGradient id="ul" x1="0" y1="0" x2="1" y2="0"><stop stopColor="#f97316" /><stop offset="1" stopColor="#ef4444" /></linearGradient></defs>
                  </svg>
                </span>
              </h1>
              ...
              {/* Trust pills — sans faux chiffres */}
              <div className={`flex flex-wrap gap-3 transition-all duration-700 delay-500 ${heroLoaded ? 'opacity-100' : 'opacity-0'}`}>
                {[
                  ...
                  { icon: MapPin, text: '100% local', bg: 'bg-blue-100 text-blue-700' },
                ].map(({ icon: I, text, bg }) => (
            ...
            <div className={`relative transition-all duration-1000 delay-400 ${heroLoaded ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'}`}>
              ...
              <div className="absolute -top-4 -right-4 bg-white rounded-2xl shadow-lg border border-gray-100 px-3 py-2 flex items-center gap-2 animate-bounce-soft">
                ...
                <div className="text-xs">
                  ...
                  <div className="text-gray-400">Réponse rapide</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-14 items-center">
            <div className="relative order-2 lg:order-1">
              ...
              <div className="absolute -bottom-5 -left-5 bg-white rounded-2xl shadow-lg border border-green-100 p-3 flex items-center gap-2.5 animate-bounce-soft">
                ...
                <div className="text-xs">
                  <div className="font-bold text-gray-800">Artisans vérifiés</div>
                  <div className="text-gray-400">manuellement ✓</div>
                </div>
              </div>
            </div>
            ...
            <div className="order-1 lg:order-2">
              ...
              <h2 className="text-4xl lg:text-5xl font-black text-gray-900 leading-tight mb-6">
                La force du réseau{' '}<span className="gradient-text">local</span>{' '}à portée de main
              </h2>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            ...
            <p className="text-gray-500 text-lg max-w-xl mx-auto">Tous les métiers dont vous avez besoin, à Biguglia et dans les communes alentours</p>
          </div>
      ...
      <section className="py-24 bg-gray-50 relative">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            ...
            <p className="text-gray-500 text-lg max-w-2xl mx-auto">Une plateforme complète pour faciliter la vie des habitants et des artisans de Biguglia</p>
          </div>
              ...
              <Link key={href} href={href}
                ...
                <div className="relative h-52 overflow-hidden bg-gray-50">
                  ...
                  <div className="absolute top-4 right-4">
                    <span className={`text-xs font-bold px-3 py-1.5 rounded-full ${accentBg} ${accentColor} border border-current/20 shadow-sm`}>{subtitle}</span>
                  </div>
            ...
            {steps.map(({ num, emoji, title, desc, color }) => (
              <div key={num} className="relative flex flex-col items-center text-center group">
                <div className={`relative w-28 h-28 rounded-3xl bg-gradient-to-br ${color} flex items-center justify-center text-5xl shadow-xl mb-6 group-hover:scale-110 transition-all duration-300 group-hover:shadow-2xl`}>
                  {emoji}
                  <div className="absolute -top-3 -right-3 w-8 h-8 bg-white rounded-full flex items-center justify-center text-xs font-black text-gray-800 shadow-md border-2 border-gray-100">{num}</div>
                </div>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {[
              { icon: Shield, title: 'Artisans vérifiés', desc: 'Chaque artisan est validé manuellement : SIRET, assurance, références. Vous êtes entre de bonnes mains.', color: 'from-orange-400 to-red-500', bg: 'bg-orange-50', border: 'border-orange-100' },
        ...
        <section className="relative py-24 overflow-hidden bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
          ...
          <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <h2 className="text-4xl sm:text-5xl lg:text-6xl font-black text-white mb-6 leading-tight">
              Rejoignez la communauté<br /><span className="gradient-text">de Biguglia</span>
            </h2>
            ...
            {/* Compteurs réels dans le CTA */}
            <div className="flex flex-wrap justify-center gap-10 pt-8 border-t border-white/10">
              {[
                { v: counts.artisans, l: 'Artisan' + (counts.artisans > 1 ? 's' : ''), loading: countsLoading },
                { v: counts.membres, l: 'Membre' + (counts.membres > 1 ? 's' : ''), loading: countsLoading },
                { v: 0, l: 'Inscription', loading: false, suffix: '€' },
              ].map(({ v, l, loading: ld, suffix: sfx }) => (
                <div key={l} className="text-center">
                  <div className="text-3xl font-black text-white">
                    {ld ? <Loader2 className="w-7 h-7 animate-spin text-gray-500 inline" /> : <>{v}{sfx || ''}</>}
                  </div>
